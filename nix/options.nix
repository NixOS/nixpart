...
let
  ...
  extraFsOptions = {
    storage = mkOption {
      type = types.nullOr deviceType;
      description = ''
        Storage device to use for this file system/mount point.
      '';
    };
  };
  ...
  options = {
    storage = {
      ...
      deviceTree = mkOption {
        ...
        internal = true;
        description = ''
          Device tree specification in JSON suitable for nixpart.
        '';
      };
    };

    fileSystems = mkOption {
      options = [ extraFsOptions ];
    };
